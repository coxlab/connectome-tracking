...
%   PredLabels = seg3D_graphical(Seg, EM, sz, numFrames, thresh, Features)
%   
%   Input:
%       Seg - Stack of labels representing 2D segmentation (double)
%       EM - Stack of original EM images (double)
%       sz - Desired image dimension (for a square image), which must be
%       less than the actual size of the original
%       numFrames - The number of frames in the stack
%       thresh - An experimentally determined threshold for clustering the
%       graph (determined by cross-validation)
%       Features - Optional argument to specify which features to use (SIFT
%       or Cox lab)
%   Output:
%       PredLabels: Stack of labels representing predicted 3D segmentation
%       (int16)
...
for i = 2:numFrames
    ...
    for j = 1:n
        ...
        Weights(unq, j) = relFreq(:)*objSize;     % TODO: make this more efficient
