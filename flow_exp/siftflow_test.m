% Computes flow between each pair of frames in a dataset
...
%	**Many constants and files currently hard-coded
...
filename = '../isbi_2013/train-labels.tif';
...
for k = 1:99
    ...
    filename1 = sprintf('../isbi_merged/pngs/train-input-norm-%02d.png', k-1);
    filename2 = sprintf('../isbi_merged/pngs/train-input-norm-%02d.png', k);
    ...
    %imshow(flow_field);
    %figure;
    b = sprintf('../isbi_merged/pngs/train-labels.tif-%02d.png', k-1);
    boundaries = imread(b);
    ...
    outfile = sprintf('../isbi_merged/pngs/flow2_train-input-norm-%02d-%02d.png', k-1, k);
    outfile2 = sprintf('../isbi_merged/pngs/flow2_overlay_train-input-norm-%02d-%02d.png', k-1, k);
...
im = CorrectPoints;
imwrite(im, '../isbi_merged/pngs/siftflow_error.png', 'PNG', 'BitDepth', 8, 'XResolution', size(im, 1), 'YResolution', size(im,2));
imwrite(warpI1, '../isbi_merged/pngs/train-input-norm-09-10_warped.png', 'PNG', 'BitDepth', 16, 'XResolution', size(im, 1), 'YResolution', size(im,2));
