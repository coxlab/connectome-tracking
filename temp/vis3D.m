function [PredLabels, GT, rand_err, VI, num_splits, num_merges] = eval_seg3D(sz, thresh, frameStart, frameEnd)
    ...
    for i = frameStart:frameEnd
        ...
        im1 = double(imread(['/n/home08/vtan/isbi_2013/pngs/train-labels-' num2str(idx-1) '.png']));
        GT(:,:,idx) = im1(1:sz,1:sz);
        im2 = double(imread(sprintf('%s/%s/pngs/%slabels.tif-%02d.png', home_dir, dataset, base_fname, idx-1)));
        Seg(:,:,idx) = im2(1:sz,1:sz);
        im3 = double(imread(sprintf([home_dir '/' dataset '/pngs/' base_fname 'input-norm-%02d.png'], idx-1)));
